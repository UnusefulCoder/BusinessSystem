	...
	class Template
	{
		...
		protected function identifyBlocks(&$line, $varname, &$queued_blocks, &$blocks)
		{
			...
			if (1 == preg_match($reg, $line, $matches)) {
				if (0 == count($queued_blocks)) $parent = $varname;
				else $parent = end($queued_blocks);
				if (!isset($blocks[$parent])) {
					$blocks[$parent] = array();
				}
				$blocks[$parent][] = $matches[1];
				$queued_blocks[] = $matches[1];
			}
		...
		{
			$result = preg_match_all("/{(" . self::$REG_NAME . ")((\-\>(" . self::$REG_NAME . "))*)?((\|.*?)*)?}/", $content, $matches);
			if ($result) {
				for ($i = 0; $i < $result; $i++) {
					// Object var detected
					if ($matches[3][$i] && (!isset($this->properties[$matches[1][$i]]) || !in_array($matches[3][$i], $this->properties[$matches[1][$i]]))) {
						$this->properties[$matches[1][$i]][] = $matches[3][$i];
					}
					// Modifiers detected
					if ($matches[7][$i] && (!isset($this->modifiers[$matches[1][$i]]) || !in_array($matches[7][$i], $this->modifiers[$matches[1][$i] . $matches[3][$i]]))) {
						$this->modifiers[$matches[1][$i] . $matches[3][$i]][] = $matches[1][$i] . $matches[3][$i] . $matches[7][$i];
					}
					// Common variables
		...
		protected function setBlock($parent, $block)
		{
			...
			if (1 !== preg_match($reg, $str, $matches)) throw new \UnexpectedValueException("mal-formed block $block");
			$this->setValue($name, '');
		...
		protected function subst($value)
		{
				...
				if (false !== strpos($string, "{" . $var . "|")) {
					foreach ($expressions as $exp) {
						if (false === strpos($var, "->") && isset($this->values['{' . $var . '}'])) {
							$string = str_replace('{' . $exp . '}', $this->substModifiers($this->values['{' . $var . '}'], $exp), $string);
						}
					}
				}
			}
			...
			foreach ($this->instances as $var => $instance) {
				foreach ($this->properties[$var] as $properties) {
					if (false !== strpos($string, "{" . $var . $properties . "}") || false !== strpos($string, "{" . $var . $properties . "|")) {
						$pointer = $instance;
						...
						if (is_object($pointer)) {
							...
							for ($i = 0; $i < count($pointer); $i++) {
								$value .= [key($pointer), current($pointer)];
