...
abstract class Conexao
{
    ...
    public static mysqli_result|bool $resultado;
...
class Usuarios extends Conexao
{
    ...
    public static function mostrar()
    {
        ...
        $maxPagina     = ceil($usuariosQuant / 10);
    ...
    public static function incluir()
    {
        ...
        try {
            if (Post::hasAll(['criarUsuario', 'criarNome', 'criarPrivilegio', 'criarSenha'])) {
                if (self::queryUsuario('SELECT', 'usuario=' .self::escape(Post::get('criarUsuario'))))
                    throw new InvalidArgumentException('Nome de usuário indisponível!');

                if (self::queryUsuario(
                    'INSERT',
                    self::validar('usuario', Post::get('criarUsuario'))
                    .', '.self::validar('nome', Post::get('criarNome'))
                    .', '.self::validar('inteiro', Post::get('criarPrivilegio'), 0, 3)
                    .', PASSWORD('.self::validar('senha', Post::get('criarSenha'))
                    .')'
                )) {
    ...
    public static function alterar()
    {
        ...
        try {
            if (Post::hasAll(['editarID', 'editarUsuario', 'editarNome', 'editarPrivilegio', 'editarSenha'])) {
                ...
                $query  = '';
                $query .= $resultado['nome'] != Post::get('editarNome')
                    ? 'nome='.self::validar('nome', Post::get('editarNome')).', ' : '';
                $query .= $resultado['privilegio'] != Post::get('editarPrivilegio')
                    ? 'privilegio='.self::validar('inteiro', Post::get('editarPrivilegio'), 0, 3).', ' : '';

                if (!strlen(Post::get('editarSenha')) == 0)
                    $query .= 'senha=PASSWORD('.self::validar('senha', Post::get('editarSenha')).'), ';
    ...
    {
        if (Get::has('id')) {
            if (self::queryUsuario('SELECT', 'id='.self::escape(Get::get('id'))) == 0)
                Session::set('status', 'Usuário não encontrado!');
            else Session::set(
                'status',
                self::queryUsuario('DELETE', 'id='.self::escape(Get::get('id'))) ? 'Sucesso!' : 'Falha!'
            );
            header('Location: ?pagina=usuarios');
            return;
        }
    ...
    public static function pesquisar()
    {
        ...
        if (is_int(Post::get('pesquisa')))
            $resultado = self::queryUsuario('SELECT', 'id='.self::escape(Post::get('pesquisa')));
        else $resultado = self::queryUsuario(
            'SELECT',
            'usuario LIKE "%'.Post::get('pesquisa').'%" OR nome LIKE "%'.Post::get('pesquisa').'%"'
        );
    ...
    public static function queryUsuario(
        $acao,
        $condicao=false,
        $campos=false,
        $comeco=false,
        ...
        if ($acao === 'SELECT') return self::query(
            'SELECT id, usuario, nome, privilegio FROM usuarios'
            .($condicao ? " WHERE $condicao" : '')
            .($ordenacao ? " ORDER BY $ordenacao" : '')
            .(is_int($comeco) && is_int($quantidade) ? " LIMIT $comeco,$quantidade" : '')
        );
        if ($acao ==='INSERT' && $condicao) return self::query(
            "INSERT INTO usuarios (usuario, nome, privilegio, senha) VALUES ($condicao)"
        );
        if ($acao === 'UPDATE' && $condicao && $campos) return self::query(
            "UPDATE usuarios SET $campos WHERE $condicao"
        );
        if ($acao === 'DELETE' && $condicao) return self::query(
            "DELETE FROM usuarios WHERE $condicao"
        );
        if ($acao === 'COUNT') {
